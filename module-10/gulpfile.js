...
gulp.task('css', () => {
  // Берем только файл main.scss в папке src, в который импортируеются паршалы
...
// Создаем таск для виброски bootstrap в папку dist/css
gulp.task('bootstrap', () => {
  return gulp.src('./src/sass/bootstrap/*.css')
  // Выкидываем bootstrap в папку dist
    .pipe(gulp.dest('./dist/css/'))
    // говорим browser-sync о том что пора перезагрузить барузер, так как файл изменился
    .pipe(browserSync.reload({
      stream: true
...
gulp.task('build', ['html', 'css', 'img', 'fonts', 'bootstrap']);
